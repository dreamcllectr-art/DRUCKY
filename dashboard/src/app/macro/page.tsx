      ...
      {tab === 'Regime' && macro && (
        <div className="space-y-6">
          ...
          <div><h2 className="text-xs text-gray-500 tracking-widest uppercase mb-3">Indicator Breakdown</h2>
            <div className="grid grid-cols-4 gap-3">{INDICATORS.map(ind => <IndicatorCard key={ind.key} name={ind.name} score={(macro as any)[ind.key]} description={ind.desc} />)}</div>
          </div>
          {topSignals.length > 0 && (<div><h2 className="text-xs text-gray-500 tracking-widest uppercase mb-3">Highest Conviction Setups</h2>
            <div className="grid grid-cols-4 gap-3">{topSignals.map((s: any) => (
